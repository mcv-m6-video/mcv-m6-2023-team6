...
def estimate_flow(motion_type, N, P, distance_type, ref_img, curr_img):
    h, w = ref_img.shape
    flow = np.zeros(shape=(h, w, 2))

    for tly_ref in range(0, h - N, N):
...
if __name__ == '__main__':
    ...
    for motion_type, N, P, distance_type in tqdm(product(args.motion_type, args.block_size, args.search_area, args.distance_type)):
        print('Estimating flow for motion_type: {}, N: {}, P: {}, distance_type: {}'.format(motion_type, N, P, distance_type))
        ...
        start = time.time()
        flow = estimate_flow(motion_type, N, P, distance_type, ref_image, curr_image)
        end = time.time()
        ...
        results.append([motion_type, distance_type, N, P, msen, pepn, end-start])

    df = pd.DataFrame(results, columns=['motion_type',  'distance_type', 'block_size', 'search_area','msen', 'pepn', 'runtime'])
