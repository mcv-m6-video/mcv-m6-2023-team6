...
def flow_liteflownet(image1, image2, colType=1, model='kitti'):
    tenOne = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(image1)[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
    tenTwo = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(image2)[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
